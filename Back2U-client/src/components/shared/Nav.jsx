...
const Nav = () => {
  ...
  return (
    <div className={`${scroll ? 'fixed top-0' : 'absolute'} transition-all duration-500 bg-[#8b8b8b58] z-50 shadow-lg w-full max-w-[89.9rem] rounded-b-2xl`}>
      <div className="sm:py-3 py-2 w-full pl-4 pr-7 navbar">
        <div className="flex items-center navbar-start">
          <div className="dropdown mt-[0.225rem] mr-2">
            ...
            <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[1] -left-2 p-2 bg-transparent w-max gap-2 font-medium transition-all">
              ...
              {/* <NavLink to='/items' className={({ isActive }) => isActive ? "px-5 py-2 bg-base-100 border-[0.1rem] border-yellow-400 rounded-lg" : "px-5 py-2 bg-base-100 border-[0.1rem] border-b-[#898888] hover:text-[#898888] border-transparent"}>Items</NavLink> */}
            </ul>
          </div>
          <div className="flex items-center gap-1">
            <img src="https://i.ibb.co/NrQ9n1Y/Black-logo-removebg-preview.png" alt="logo" className="h-6" />
        ...
        <div className="hidden lg:flex min-w-max navbar-center">
          <ul className="font-medium gap-4">
            ...
            {/* <NavLink to='/items' className={pages}>Items</NavLink> */}
          </ul>
        </div>
        <div id="nav-btn" className="flex gap-4 items-center navbar-end">

          {user && <div className="avatar dropdown">
            ...
            <ul tabIndex={0} className="dropdown-content top-14 right-0  z-[1] menu p-2 shadow-md shadow-yellow-400 bg-base-100 rounded-lg min-w-max transition-all">
              <p className="font-medium mb-2">Hi, <span className="uppercase">{user?.displayName.split(' ')[user?.displayName.split(' ').length - 1]}</span></p>
